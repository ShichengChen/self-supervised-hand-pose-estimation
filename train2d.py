...
for epoch in range(80):
    ...
    for idx, inp in enumerate(train_loader):
        ...
        dicloss = {"loss": float(loss), "epe": float(eucLoss) * 1000, "epe loss": float(pose3dloss),
                   "bio dis": float(bioEudloss) * 1000, "bio loss": float(bioLoss),
                   "bone dis": float(eucLoss_bone) * 1000, "bone loss": float(poseloss_bone),
                   "pixel dis": float(pixelLoss), "2d loss": float(pose2dloss)*0.001,
                   "Greedymatchdis dis": float(Greedymatchdis) * 1000,
                   }
    ...
    torch.save({
        ...
        'optimizer': optimizer.state_dict()}, './models/'+platform.node()+'rgb2pose2d.pt')
